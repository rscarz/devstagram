...
@section('contenido')
        ...
        <div class="w-full md:w-8/12 lg:w-6/12 flex flex-col items-center md:flex-row">
            ...
            <div class="md:w-8/12 lg:w-6/12 px-5 flex flex-col items-center md:justify-center md:items-start py-10 md:py-10">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @auth
                        {{-- si el usuario es distinto al logueado le muestra para seguir y dejar de seguir --}}
                        @if ($user->id !== auth()->user()->id)
                            @if ( !$user->siguiendo( auth()->user() ) )
                                
                            
                                <form action="{{ route('user.follow' , $user) }}"
                                    method="POST" >
                                    @csrf
                                    
                                    <input
                                        type="submit"
                                        class="bg-blue-400 text-white uppercase rounded-lg px-3 py-1 text-xs font-bold cursor-pointer hover:bg-blue-900"
                                        value="Seguir"
                                    />
                                </form>
                            @else
                                <form action="{{ route('user.unfollow' , $user) }}"
                                method="POST" >
                                    @csrf
                                    @method('DELETE')
                                    ...
                                    <input
                                        type="submit"
                                        class="bg-red-600 text-white uppercase rounded-lg px-3 py-1 text-xs font-bold cursor-pointer hover:bg-red-900"
    ...
    <section class="container mx-auto mt-10 px-10">
